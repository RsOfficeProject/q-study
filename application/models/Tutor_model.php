...
class Tutor_model extends CI_Model
{
    ...
    {
        $this->db->select('*');
        $this->db->from('tbl_new_modulequestion');
        $this->db->join('tbl_new_module', 'tbl_new_modulequestion.module_id = tbl_new_module.id', 'LEFT');
        $this->db->join('tbl_moduletype', 'tbl_moduletype.id = tbl_new_module.moduleType', 'LEFT');
        $this->db->join('tbl_question', 'tbl_question.id = tbl_new_modulequestion.question_id', 'LEFT');
        $this->db->where('tbl_new_modulequestion.module_id', $id);
        
        if ($status == null) {
            $this->db->where('tbl_new_modulequestion.question_order', $question_order_id);
        } else {
            $this->db->order_by("question_order", "asc");
        }
